...
from_whatsapp_number = Variable.get("twilio_whatsapp_number")
to_whatsapp_number = Variable.get("recipient_whatsapp_number")

def get_weather_data_and_send_to_whatsapp(country, city):
    ...
    if response.status_code == 200:
        ...
        #Send the message to WhatsApp using Twilio
...
dag = DAG(
    "weather_forecast_fr_lt",
    default_args=default_args,
    schedule_interval="0 8 * * *",  # Run daily at 08:00 AM UTC
    catchup=False,
)

get_weather_data_paris = PythonOperator(
    task_id="get_weather_data_paris",
    python_callable=get_weather_data_and_send_to_whatsapp,
    op_args=["FR", "Paris"],
    dag=dag,
)
